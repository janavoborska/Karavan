...
(function (root, window, document, factory, undefined) {
  ...
  function showError(type, text) {
    window.console &&
      window.console[type] &&
      window.console[type]('TavoCalendar: ' + text);
  }
  ...
  function getDummyDay() {
    ...
    dummy_day_wrapper_el.className =
      CLASS_CALENDAR_DAY + ' ' + CLASS_CALENDAR_DAY_DIFFERENT_MONTH;
  ...
  TavoCalendar.prototype.mount = function () {
    ...
    // Info
    var calendar_reset_el,
      calendar_select_date_el,
      calendar_select_date_start_el,
      calendar_select_date_end_el;

    // Code Header
    var calendar_header_el,
      calendar_month_el,
      calendar_nav_prev_el,
      calendar_nav_next_el;
    ...
    if (this.state.lock) {
      calendar_code_el.className =
        CLASS_CALENDAR_CODE + ' ' + CLASS_CALENDAR_CODE_LOCK;
    } else {
    ...
    calendar_nav_prev_el.className =
      CLASS_CALENDAR_NAV_PREV + ' ' + CLASS_CALENDAR_NAV;
    ...
    calendar_nav_next_el.className =
      CLASS_CALENDAR_NAV_NEXT + ' ' + CLASS_CALENDAR_NAV;
    ...
    let offset =
      moment_copy.isoWeekday() % (7 + this.locale_data.firstDayOfWeek());
    ...
    for (var d = 1; d <= days_in_month; d++) {
      ...
      if (moment_copy.isSame(moment(), 'day')) {
        day_wrapper_el.className =
          day_wrapper_el.className + ' ' + CLASS_CALENDAR_DAY_ABS_TODAY;
      } else if (moment_copy.isBefore(moment(), 'day')) {
        day_wrapper_el.className =
          day_wrapper_el.className + ' ' + CLASS_CALENDAR_DAY_ABS_PAST;
      } else if (moment_copy.isAfter(moment(), 'day')) {
        day_wrapper_el.className =
          day_wrapper_el.className + ' ' + CLASS_CALENDAR_DAY_ABS_FUTURE;
      }

      //RELATIVE RELATIONS
      if (
        moment_copy.isSame(moment(this.state.date, this.config.format), 'day')
      ) {
        day_wrapper_el.className =
          day_wrapper_el.className + ' ' + CLASS_CALENDAR_DAY_REL_TODAY;
      } else if (
        moment_copy.isBefore(moment(this.state.date, this.config.format), 'day')
      ) {
        day_wrapper_el.className =
          day_wrapper_el.className + ' ' + CLASS_CALENDAR_DAY_REL_PAST;
      } else if (
        moment_copy.isAfter(moment(this.state.date, this.config.format), 'day')
      ) {
        day_wrapper_el.className =
          day_wrapper_el.className + ' ' + CLASS_CALENDAR_DAY_REL_FUTURE;
      }

      //MEANINGFUL MARKERS
      if (
        this.state.highlight.indexOf(moment_copy.format(this.config.format)) >
        -1
      ) {
        day_wrapper_el.className =
          day_wrapper_el.className + ' ' + CLASS_CALENDAR_DAY_HIGHTLIGHT;
      }
      ...
      if (
        this.state.blacklist.indexOf(moment_copy.format(this.config.format)) >
        -1
      ) {
        day_wrapper_el.className =
          day_wrapper_el.className +
          ' ' +
          CLASS_CALENDAR_DAY_OFF +
          ' ' +
          CLASS_CALENDAR_DAY_LOCK;
      }

      if (
        this.state.selected.indexOf(moment_copy.format(this.config.format)) > -1
      ) {
        day_wrapper_el.className =
          day_wrapper_el.className + ' ' + CLASS_CALENDAR_DAY_SELECT;
      }

      //Range select
      if (
        this.state.date_start &&
        moment_copy.isSame(this.state.date_start, 'day')
      ) {
        day_wrapper_el.className =
          day_wrapper_el.className +
          ' ' +
      ...
      if (
        this.state.date_start &&
        !this.state.date_end &&
        moment_copy.isBefore(
          moment(this.state.date_start, this.config.format),
          'day',
        )
      ) {
        day_wrapper_el.className =
          day_wrapper_el.className + ' ' + CLASS_CALENDAR_DAY_LOCK;
      }

      if (
        this.state.date_end &&
        moment_copy.isSame(this.state.date_end, 'day')
      ) {
        day_wrapper_el.className =
          day_wrapper_el.className +
          ' ' +
      ...
      if (
        (moment_copy.isBefore(
          ...
          !this.config.past_select) ||
        (moment_copy.isAfter(
          moment(this.state.date, this.config.format),
          'day',
        ) &&
          !this.config.future_select)
      ) {
        day_wrapper_el.className =
          day_wrapper_el.className + ' ' + CLASS_CALENDAR_DAY_LOCK;
      }
    ...
    if (this.state.day) {
      calendar_select_date_el.textContent = this.moment.format(
        this.config.format,
      );
    }
